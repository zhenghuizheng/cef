...
namespace base {
	...
	HistogramBase* Histogram::DeserializeInfoImpl(PickleIterator* iter) {
		...
		HistogramBase* histogram = Histogram::FactoryGet(
			histogram_name, declared_min, declared_max, static_cast<size_t>(bucket_count), flags);
	...
	HistogramBase* LinearHistogram::FactoryTimeGet(const string& name,
		...
		size_t bucket_count,
		int32 flags) {
		return FactoryGet(name, static_cast<Sample>(minimum.InMilliseconds()), static_cast<Sample>(maximum.InMilliseconds()),
			bucket_count, flags);
	}
	...
	HistogramBase* CustomHistogram::DeserializeInfoImpl(PickleIterator* iter) {
		...
		vector<Sample> sample_ranges(static_cast<unsigned int>(bucket_count - 1));
